...
const levelThreePlatforms: Platform[] = [
  ...
  { x: 5525, y: 214, width: 300, height: 34, kind: "snow" },
];
...
const levelFourPlatforms: Platform[] = [
  ...
  { x: 2165, y: -1008, width: 520, height: 54, kind: "snow" },
  { x: 3135, y: -560, width: 420, height: 54, kind: "ice" },
];
...
export class PuffinGame {
  ...
  private updatePenguins(deltaSeconds: number): void {
    ...
    for (const penguin of this.penguins) {
      ...
      if (chasingPuffin && Math.abs(distanceX) > turnDeadZone) {
      ...
      if (this.isPenguinFacingHazard(penguin)) {
        penguin.direction *= -1;
  ...
  private checkPenguins(previousY: number): void {
    for (const penguin of this.penguins) {
      ...
      this.damagePuffin(puffinCenter < penguinCenter ? -1 : 1, pickaxeHit ? 2 : 1);
      return;
    }
  }
  ...
  private startSelectedLevel(levelNumber: number): void {
    ...
    this.currentLevel = clamp(Math.round(levelNumber), 1, 5);
    this.levelStartFeathers = this.puffinFeathers;
  ...
  private updateLevelsDisplay(): void {
    ...
    for (const levelButton of this.levelButtons) {
      ...
      const completed = this.completedLevels.has(levelNumber);
      const current = levelNumber === currentUnlockedLevel && !completed;
      const status = levelButton.querySelector("span");

      levelButton.classList.toggle("completed", completed);
      levelButton.classList.toggle("current-level", current);
      levelButton.classList.toggle("not-completed", !completed && !current);
      levelButton.disabled = false;

      if (status) {
        status.textContent = completed ? "Completed" : current ? "Current" : "Unlocked";
      }
    }
  }

  private getCurrentUnlockedLevel(): number {
    if (!this.completedLevels.has(1)) {
      return 1;
    }

    if (!this.completedLevels.has(2)) {
      return 2;
    }
  ...
  private drawPuffin(): void {
    ...
    this.context.fillStyle = "#f7fbff";
    this.context.beginPath();
    this.context.ellipse(12, -31, 15, 19, 0, 0, Math.PI * 2);
    this.context.fill();

    this.context.fillStyle = "#f08f3c";
    this.context.fillRect(21, -39, 18 + peckReach, 10);

    this.context.fillStyle = "#f3d74f";
    this.context.fillRect(29, -36, 10 + peckReach, 4);

    this.context.fillStyle = "#d83a2e";
    this.context.fillRect(21, -29, 16 + peckReach, 7);
  ...
  private updateExternalHud(): void {
    ...
    if (this.hudStats) {
      this.hudStats.textContent = showingOnlyFeathers
        ...
        : `Level ${this.currentLevel} · Fish ${this.score}/${this.fish.length} · Penguins ${this.penguinsDefeated}/${this.penguins.length} · Feathers ${this.puffinFeathers}`;
    }
  ...
  private drawLevelTitleCard(): void {
    ...
    const size = VIEW.height * 0.75;
    const x = VIEW.width / 2 - size / 2;
    const y = VIEW.height / 2 - size / 2;
    ...
    this.context.lineWidth = 5;
    this.context.strokeRect(x + 3, y + 3, size - 6, size - 6);

    this.context.fillStyle = "#12303d";
    this.context.font = "900 64px system-ui, sans-serif";
    this.context.textAlign = "center";
    this.context.textBaseline = "middle";
    this.context.fillText(`Level ${this.currentLevel}`, VIEW.width / 2, y + size / 2);
    this.context.restore();
  }
...
function isGameKey(key: string): boolean {
  return ["arrowleft", "arrowright", "arrowup", "arrowdown", "a", "d", "s", "w", " ", "r", "e"].includes(
    key.toLowerCase(),
  );
}
